...
async function main() {
    try {
        ...
        const caURL = ccp.certificateAuthorities['ca.vol1.example.com'].url;
        ...
        const userIdentity = await volent.get('appUser');
        if (userIdentity) {
        ...
        const adminIdentity = await volent.get('admin');
        if (!adminIdentity) {
        ...
        const provider = volent.getProviderRegistry().getProvider(adminIdentity.type);
        ...
        await volent.put('appUser', x509Identity);
